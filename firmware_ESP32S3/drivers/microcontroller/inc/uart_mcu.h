...
typedef enum uart_ports{
	UART_PC,				/*!< UART0, uses whatever TX/RX pins ESP-IDF has configured by default for the console (native USB-Serial-JTAG, or UART0 on GPIO43/GPIO44, depending on the sdkconfig) - no fixed pin of its own */
	UART_CONNECTOR,			/*!< UART1 (TX: GPIO17, RX: GPIO18) */
} uart_mcu_port_t;
/**
